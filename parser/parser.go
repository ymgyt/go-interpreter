...
func (p *Parser) parseIntegerLiteral() ast.Expression {
	...
	if err != nil {
		p.errors = append(p.errors, newParseError("could not parse %q as interger", p.curToken.Literal))
	}
	lit.Value = value
...
func (p *Parser) parseFunctionParameters() []*ast.Identifier {
	...
	if p.peekTokenIs(token.RPAREN) { // func()
		p.nextToken()
		return nil
	}
	...
	for p.peekTokenIs(token.COMMA) {
		p.nextToken() // consume func(a <- ,b)
		p.nextToken() // consume func(a, <- b)
		ident := &ast.Identifier{Token: p.curToken, Value: p.curToken.Literal}
		identifiers = append(identifiers, ident)
	}

	if !p.expectPeek(token.RPAREN) { // consume func(a,b) <-
		return nil
	}
	return identifiers
}
...
func (p *Parser) peekError(t token.TokenType) {
	p.errors = append(p.errors, newParseError("unexpected next token. want %s, got %s", t, p.peekToken.Type))
}

func (p *Parser) noPrefixParseFnError(t token.TokenType) {
	p.errors = append(p.errors, newParseError("no prefix parse function for %s found", t))
}
